import { SafeAreaView, ScrollView, TextInput, TouchableOpacity, useColorScheme, View, Image } from 'react-native'
    ...
    async function fetchMeals() {
        ...
        if (selectedOption === 'All') {
            meals = await DataStore.query(Meal, x => x.and(m => [
                ...
                m.public.eq(true)
            ]), { sort: x => x.name('ASCENDING'), limit: 20 })
        } else if (selectedOption === 'From Pantry') {
            const pantryIngredients = await DataStore.query(PantryItem, pi => pi.and(x => [x.userID.eq(userId), x.purchased.eq(true)]))
            ...
            meals = await DataStore.query(Meal, x => x.and(y => [y.userID.eq(userId), debouncedSearchTerm ? y.name.contains(debouncedSearchTerm) : y.name.ne(''), y.public.eq(true)]), { sort: x => x.name('ASCENDING'), limit: 20 })
        }
        const mealResults: ListMealSearchResults[] = await Promise.all(meals.map(async meal => {
            ...
            return {
                name: meal.name,
                id: meal.id,
                img: isStorageUri(picture.uri) ? await Storage.get(picture.uri) : picture.uri,
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={[tw`px-4 pb-20`]}>
                ...
                {(displaySearchState !== '' && results.length === 0) && <View style={tw`w-12/12 justify-center items-center mt-15`}><Text style={tw`text-xl`} weight='bold'>{displaySearchState}</Text></View>}
                {results.map((r, idx) => {
                    return <TouchableOpacity
                        ...
                        {/* @ts-ignore */}
                        <Image source={{ uri: r.img }} style={tw`h-20 w-20 rounded-lg`} resizeMode='cover' />
                        <View style={tw`px-2 w-9/12 max-w-9/12`}>
                            <Text style={tw`text-lg`} weight='bold'>{r.name}</Text>
                            <Text>By {<Text style={tw`text-red-500`}>{r.author}</Text>}</Text>
                            <View style={tw`w-12/12 py-1 rounded-xl`}>
                            <Text>{r.ingredientCount} ingredients & {r.stepCount} steps</Text>
                            <Text>{r.calories.toFixed()} kcal | P: {r.protein.toFixed()}g | C: {r.carbs.toFixed()}g | F: {r.fat.toFixed()}g</Text>
                            </View>
                        </View>
                    </TouchableOpacity>
                })}
